...
class RecycleBinController extends Controller
{
    public function recycle_bin_product(){
        if(session('username_admin') && session('id_admin')){
            ...
            $recycle_product=DB::table('product')->join('photo','photo.id_product','=','product.id_product')->join('category','category.id','=','product.id_category')->whereNotNull('product.deleted_at')->where('photo.status','=',1)->get();
    ...
    public function recycle_bin_category(){
        if(session('username_admin') && session('id_admin')){
            $data_last = session()->get('value_admin');
            $list_photo = DB::table('user')->select('*')->where('avatar', $data_last)->first();
            $recycle_category=DB::table('category')->whereNotNull('category.deleted_at')->get();
    ...
    public function recycle_bin_photo(){
        if(session('username_admin') && session('id_admin')){
            $data_last = session()->get('value_admin');
            $list_photo = DB::table('user')->select('*')->where('avatar', $data_last)->first();
            $recycle_photo=DB::table('photo')->join('product','product.id_product','=','photo.id_product')->whereNotNull('photo.deleted_at')->get();
